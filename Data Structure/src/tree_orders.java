...
public class tree_orders {
	...
	public class TreeOrders {
		...
		void preMethod(int i, ArrayList<Integer> result) {
			
			if(left[i] != -1) {
				result.add(key[left[i]]);
				preMethod(left[i], result);
			}
			
			if(right[i] != -1) {
				result.add(key[right[i]]);
				preMethod(right[i], result);
			}
		}
